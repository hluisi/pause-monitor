...
class TestLibprocCollectorRogueSelection:
    """Test that LibprocCollector uses same rogue selection logic as TopCollector."""

    def test_select_rogues_stuck_always_included(self):
        """Stuck processes are always selected (same as TopCollector)."""
        config = Config()
        collector = LibprocCollector(config)
    ...
    def test_score_process_matches_top_collector(self):
        """Scoring produces same result as TopCollector for identical input."""
        config = Config()
        top_collector = TopCollector(config)
        libproc_collector = LibprocCollector(config)
        ...
        top_score = top_collector._score_process(proc)
        libproc_score = libproc_collector._score_process(proc)

        assert top_score.score == libproc_score.score
        assert top_score.categories == libproc_score.categories
